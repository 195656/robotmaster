...
while True:
    ...
    # OpenCV 4 兼容写法：始终取返回值的最后两个
    contours_result = cv2.findContours(frame_thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    contours = contours_result[-2]  # contours 总是倒数第二个返回值
    hierarchy = contours_result[-1]  # hierarchy 总是最后一个返回值
